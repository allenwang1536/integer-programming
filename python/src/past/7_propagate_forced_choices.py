...
class IPInstance:
    ...
    def branch_and_bound(self, fixed_zero, fixed_one, precomputed_result=None):
        original_fixed_zero = set(fixed_zero)
        original_fixed_one = set(fixed_one)

        fixed_zero, fixed_one = self.propagate_forced_choices(fixed_zero, fixed_one)
        
        if self.forced_one_cost(fixed_one) >= self.best_objective:
            return
            
        if fixed_zero != original_fixed_zero or fixed_one != original_fixed_one:
