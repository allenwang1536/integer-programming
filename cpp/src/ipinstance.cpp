...
LPResult IPInstance::SolveLPRelaxation(const std::vector<bool>& fixed_zero,
  ...
  MPSolver solver("healthcare_lp", MPSolver::SCIP_MIXED_INTEGER_PROGRAMMING);
...
bool IPInstance::PropagateForcedChoices(std::vector<bool>& fixed_zero,
  ...
  while (changed) {
    ...
    for (const PairConstraint& pair : pair_constraints_) {
      ...
      int last_available_test = -1;

      for (int test_id : pair.distinguishing_tests) {
        if (fixed_one[test_id]) {
          already_covered = true;
          break;
        }
        if (!fixed_zero[test_id]) {
          ++available_count;
          last_available_test = test_id;
        }
      }
      ...
      if (available_count == 1) {
        if (fixed_zero[last_available_test]) {
          return false;
        }
        if (!fixed_one[last_available_test]) {
          fixed_one[last_available_test] = true;
          changed = true;
        }
      }
    }
...
void IPInstance::BranchAndBound(const std::vector<bool>& fixed_zero,
  ...
  struct OpenNode {
    double bound;
    std::size_t tie_breaker;
    std::vector<bool> fixed_zero;
    std::vector<bool> fixed_one;
    LPResult lp_result;
  };
  ...
  std::priority_queue<OpenNode, std::vector<OpenNode>, OpenNodeCompare> open_nodes;
  std::size_t next_tie_breaker = 0;

  auto push_node = [&](std::vector<bool> node_fixed_zero,
                       std::vector<bool> node_fixed_one) {
    if (!PropagateForcedChoices(node_fixed_zero, node_fixed_one)) {
      return;
    }

    if (ForcedOneCost(node_fixed_one) >= best_objective_ - kEps) {
      return;
    }

    const LPResult result = SolveLPRelaxation(node_fixed_zero, node_fixed_one);
    if (!result.feasible) {
      return;
    }
    ...
    open_nodes.push(OpenNode{
        ...
        next_tie_breaker++,
        std::move(node_fixed_zero),
        std::move(node_fixed_one),
        result,
    });
  };

  push_node(fixed_zero, fixed_one);

  while (!open_nodes.empty()) {
    OpenNode current = open_nodes.top();
    open_nodes.pop();

    if (ForcedOneCost(current.fixed_one) >= best_objective_ - kEps) {
      continue;
    }

    if (current.lp_result.objective_value >= best_objective_ - kEps) {
      continue;
    }

    if (current.lp_result.is_integral) {
      best_objective_ = current.lp_result.objective_value;
      best_solution_ = ExtractIntegerSolution(current.lp_result.x_values);
      has_best_solution_ = true;
      continue;
    }

    const GreedyResult heuristic = HeuristicSolutionFromLP(
        current.lp_result.x_values, current.fixed_zero, current.fixed_one);
    ...
    const int branch_var =
        ChooseBranchVariable(current.lp_result.x_values, current.fixed_zero, current.fixed_one);
    if (branch_var == -1) {
      continue;
    }

    std::vector<bool> left_zero = current.fixed_zero;
    left_zero[branch_var] = true;
    push_node(std::move(left_zero), current.fixed_one);

    std::vector<bool> right_one = current.fixed_one;
    right_one[branch_var] = true;
    push_node(current.fixed_zero, std::move(right_one));
  }
}
